...
const Delete = ({id,action}) => {
   ...
   const deleteBootcamp = async()=>{
     ...
     try{
        ...
        showPopup({success:true,message:"Bootcamp deleted successfully!"})
        action(true);
     }catch(err){
        console.error(err);
  ...
  return (
    <div className="fixed  inset-0 bg-gray-800 bg-opacity-75 flex justify-center items-center  ">
      <div className="bg-white w-max p-4 rounded-lg h-36 grid gap-6 ">
         <p className='font-semibold text-lg px-4'>Do you want delete this bootcamp?</p>
         <p className='w-full flex space-x-4'>
            <button className='w-full py-2 bg-red-500 border-2 border-red-500 hover:bg-red-400 text-white rounded-md' onClick={()=>deleteBootcamp()}>Yes</button>
